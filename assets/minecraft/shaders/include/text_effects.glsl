...
void apply_skewing_movement(float speed)
{
    ...
    textData.uv.y = mix(textData.uv.y, textData.uvMax.y, -(0.15 * cos(TAU * t)));
    textData.shouldScale = true;
}
...
void apply_shimmer(float speed, float intensity)
{
    ...
    if (mod(f, 15) < 0.7) textData.topColor = vec4(1.0, 1.0, 1.0, intensity);
}

void apply_shimmer()
{
    apply_shimmer(0.9, 0.5);
}
...
void apply_animated_gradient_7(vec3 color1, vec3 color2, vec3 color3, vec3 color4, vec3 color5, vec3 color6, vec3 color7)
{
    ...
    vec3 resultColor;
    if (smoothT < 1.0) resultColor = mix(color1, color2, smoothT);
    else if (smoothT < 2.0) resultColor = mix(color2, color3, smoothT - 1.0);
    else if (smoothT < 3.0) resultColor = mix(color3, color4, smoothT - 2.0);
    else if (smoothT < 4.0) resultColor = mix(color4, color5, smoothT - 3.0);
    else if (smoothT < 5.0) resultColor = mix(color5, color6, smoothT - 4.0);
    else if (smoothT < 6.0) resultColor = mix(color6, color7, smoothT - 5.0);
    else if (smoothT < 7.0) resultColor = mix(color7, color6, smoothT - 6.0);
    else if (smoothT < 8.0) resultColor = mix(color6, color5, smoothT - 7.0);
    else if (smoothT < 9.0) resultColor = mix(color5, color4, smoothT - 8.0);
    else if (smoothT < 10.0) resultColor = mix(color4, color3, smoothT - 9.0);
    else if (smoothT < 11.0) resultColor = mix(color3, color2, smoothT - 10.0);
    else resultColor = mix(color2, color1, smoothT - 11.0);
...
bool applySpheyaPack9()
{
    ...
    if (textData.isShadow)
    {
        textData.color.a = originalAlpha;
    }
